...
describe('ClaudeGenerator', () => {
  describe('Template Loading', () => {
    it('should load CLAUDE.md template content', async () => {
      const generator = new ClaudeGenerator();
      const templateContent = await generator.loadTemplate('CLAUDE.md');
  ...
  describe('Instructions Directory Copying', () => {
    it('should copy instructions directory structure', async () => {
      const generator = new ClaudeGenerator();
      const instructionFiles = await generator.getInstructionFiles();
      ...
      expect(instructionFiles).toContain('develop.md');
      expect(instructionFiles).toContain('KentBeck-tdd-rules.md');
    });
  });
  ...
  describe('File Generation', () => {
    ...
    it('should generate CLAUDE.md and instructions directory in target location', async () => {
      const generator = new ClaudeGenerator();
      await generator.generateFiles(testOutputDir, { projectName: 'test-project' });
    ...
    it('should replace template variables in generated files', async () => {
      ...
      const generator = new ClaudeGenerator();
      await generator.generateFiles(testOutputDir, { projectName: 'my-awesome-project' });
